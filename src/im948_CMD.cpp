...
void im948ReadData(void) {
    ...
    if (isNewData) { // 已更新数据
        isNewData = 0;
        Serial.printf("AngleX: %f \n", AngleX);
        Serial.printf("AngleY: %f \n", AngleY);
 ...
 *
 * @param Flag
 * 取值2=开机时载入配置参数并开启串口主动上报，1=开机时载入配置参数但串口不主动上报,
 * 0=不载入且串口不主动上报
 */
void Cmd_40(U8 Flag) {
    U8 buf[2] = {0x40};
...
F32 AngleX, AngleY, AngleZ, Ax, Ay, Az, Gx, Gy, Gz, Temperature, airPress,
    Height; // 从Cmd_RxUnpack中获取到的欧拉角数据等更新到全局变量以便用户自己的业务逻辑使用,
            // 若还需要其它数据，可参考进行增加即可
...
static void Cmd_RxUnpack(U8 *buf, U8 DLen) {
    ...
    case 0x10: // 模块当前的属性和状态 回复
        ...
        Dbp("\t battery voltage: %u mv\r\n",
            (U16)(((U16)buf[14] << 8) |
                  buf[13])); // 字节[14-13] 电池的当前电压mv
        Dbp("\t Mac: %02X:%02X:%02X:%02X:%02X:%02X\r\n", buf[15], buf[16],
            buf[17], buf[18], buf[19], buf[20]); // 字节[15-20] MAC地址
    ...
    case 0x25: // 读取关机电压和充电参数 回复
        ...
        Dbp("\t charge_full_mA: %u ma\r\n",
            buf[3]); // 字节3 充电截止电流 0:2ma 1:5ma 2:7ma 3:10ma 4:15ma
                     // 5:20ma 6:25ma 7:30ma
        Dbp("\t charge_mA: %u ma\r\n",
 ...
 */
static void Cmd_Write(U8 *pBuf, int Len) {
    im948_serial.write(pBuf, Len); // 通过该函数发送通信数据流
    Dbp_U8_buf("tx: ", "\r\n", "%02X ", pBuf, Len);
}

// ======================================测试示例==============================================
U8 im948_ctl =
    0; // 用户调试口发来的1字节操作指令 调试口收到的数据赋值给im948_ctl
       // 主循环里调用 im948_test() 进行演示
